...
def train() -> float:
    ...
    for batch in tqdm(train_loader, total=total_steps):
        ...
        x_src, schema_node_features, ori_node_features, bases = model(src_batch, task.src_entity_table, Sch_data, table_to_index)
        x_pos_dst, schema_node_features, ori_node_features, bases = model(batch_pos_dst, task.dst_entity_table, Sch_data, table_to_index)
        x_neg_dst, schema_node_features, ori_node_features, bases = model(batch_neg_dst, task.dst_entity_table, Sch_data, table_to_index)
        ...
        loss_tsk = F.softplus(-diff_score).mean()
...
def test(src_loader: NeighborLoader, dst_loader: NeighborLoader) -> np.ndarray:
    ...
    for batch in tqdm(dst_loader):
        ...
        emb, schema_node_features, ori_node_features, bases = model(batch, task.dst_entity_table, Sch_data, table_to_index)
        emb = emb.detach()
        dst_embs.append(emb)
    ...
    for batch in tqdm(src_loader):
        batch = batch.to(device)
        emb, schema_node_features, ori_node_features, bases = model(batch, task.src_entity_table, Sch_data, table_to_index)
