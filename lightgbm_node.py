...
if task.task_type in [TaskType.BINARY_CLASSIFICATION, TaskType.REGRESSION]:
    ...
    for i in tqdm(range(task.num_labels)):
        ...
        pred_train_list.append(model.predict(tf_test=tf_train).cpu().numpy())
        pred_val_list.append(model.predict(tf_test=tf_val).cpu().numpy())
        pred_test_list.append(model.predict(tf_test=tf_test).cpu().numpy())
...
#python lightgbm_node.py --dataset rel-f1 --task driver-position
