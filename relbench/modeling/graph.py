...
def make_schema_graph(
    ...
    num_nodes = len(db.table_dict)  # 节点总数
    x = [] # 用于存储节点特征
    edge_index = [[], []] # 用于存储边的索引
    ...
    for table_index, (table_name, table) in enumerate(db.table_dict.items()): 
        table_to_index[table_name] = table_index
        index_to_table[table_index] = table_name
        x.append(word_embedder(table_name))
    ...
    for table_name, table in db.table_dict.items(): 
        ...
        for fkey_name, pkey_table_name in table.fkey_col_to_pkey_table.items():
            pkey_index = df[fkey_name] # 获取外键列的值，即主键表的索引
            # fkey -> pkey edges
            
            edge_index[0].append(table_to_index[table_name])
            edge_index[1].append(table_to_index[pkey_table_name])

            # pkey -> fkey edges.
            edge_index[0].append(table_to_index[pkey_table_name])
            edge_index[1].append(table_to_index[table_name])
    ...
    return data, index_to_table, table_to_index, adj_matrix
...
def make_pkey_fkey_graph(
    ...
    for table_name, table in db.table_dict.items():
        ...
        for fkey_name, pkey_table_name in table.fkey_col_to_pkey_table.items():
            ...
            #print(pkey_index.shape, fkey_index.shape)
            # fkey -> pkey edges
